...
module.exports = 
{
	...
	*_categoriza( libro ) 
	...
	/**
	 * Crea un nuevo Stream para recibir el nuevo ebook y lo guarda en el path para los nuevos Ebooks
	 * @param	{string}	nombre
	 * @returns	{object}	writeStream
	 */
	adicionaNuevoLibro( nombre )
	{
		console.assert( typeof nombre === 'string', 'El nombre del libro', nombre);

		let ebookPath = `${ EBOOKS }/${ nombre }`;

		return fs.createWriteStream( ebookPath, { defaultEncoding: null });
	},
};
