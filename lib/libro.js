...
module.exports = 
{
     ...
     * @param {object} actual Objeto con toda la informacion del libro
     */
    actualizaPaginaActual( nombre, actual ) 
	{
		this.totalInfo[ nombre ] = actual;
	...
	*_categoriza( libro ) 
	...
	/**
	 * Actualiza los comentarios que tenemos del libro
	 * @param	{String}	libre	El nombre del libro a calificar
	 * @param	{Number}	notas
	 */
	comenta({ libro, notas }) 
	{
		console.assert( libro, 'Debe venir el nombre del libro', libro );
		console.assert( notas, 'Si viene la notas debe ser mayor a 0', notas );

		if ( 'totalInfo' in this && this.totalInfo[ libro ]) 
		{
            this.totalInfo[ libro ].notas = notas;
	...
	// TODO: Documentar
	adicionaNuevoLibro( nombre, contenido )
	{
		console.assert( typeof nombre === 'string', 'El nombre del libro', nombre);
		console.assert( typeof contenido === 'object' && contenido.length, 'El contenido debe ser un buffer', contenido);

		let nuevoEbook = `${ EBOOKS }/${ nombre }`;

		fs.writeFile( nuevoEbook, contenido, { encoding: null }, error =>
		{
			if (error) console.log( error );

			console.log( 'Terminamos de subir el libro ' + nombre );
		});
	},
};
